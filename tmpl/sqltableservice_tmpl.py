...
class {{ name }}Service(object):
    ...
    def getall_{{ name }}(self):
        try:
            ...
            with Session(engine) as session:
                results = session.query({{ name }}).limit(configindb.Query_Config['query_limit_upset'])
                return results.all()
        except Exception as e:
            log.error('Exception at getall_{{ name }}(): %s ' % e)
    ...
    def get_{{ name }}_count(self):
        try:
            ...
            log.error('Exception at get_{{ name }}_count(): %s ' % e)
            if config('app_exception_detail', cast=bool, default=True):
                traceback.print_exc()
            return None
        finally:
    ...
    def create_{{ name }}(self, modelobj):
        try:
            engine = dbengine.DBEngine().connect()
            with Session(engine) as session:
            ...
            log.error('Exception at create_{{ name }}(): %s ' % e)
            if config('app_exception_detail', cast=bool, default=True):
                traceback.print_exc()
            return None
        finally:
    ...
    def batch_create_{{ name }}_byjson(self, jsonstr):
        ...
        try:
            ...
            log.error('Exception at batch_create_{{ name }}(): %s ' % e)
            if config('app_exception_detail', cast=bool, default=True):
                traceback.print_exc()
            return None
    ...
    def get_{{ name }}_byid(self, idstr):
        try:
            ...
            with Session(engine) as session:
                ...
                #log.debug('get_{{ name }}_byid() result is : %s' % result)
                return result
        except Exception as e:
            log.error('Exception at get_{{ name }}_byid(): %s ' % e)
            if config('app_exception_detail', cast=bool, default=True):
                traceback.print_exc()
            return None
        finally:
    ...
    def update_{{ name }}(self, modelobj):
        try:
            engine = dbengine.DBEngine().connect()
            with Session(engine) as session:
                session.add(modelobj)
            ...
            log.error('Exception at update_{{ name }}(): %s ' % e)
            if config('app_exception_detail', cast=bool, default=True):
                traceback.print_exc()
            return None
        finally:
    ...
    def update_{{ name }}_byjson(self, updatejson):
        ...
        try:
            ...
            log.error('Exception at update_{{ name }}_byjson(): %s ' % e)
            if config('app_exception_detail', cast=bool, default=True):
                traceback.print_exc()
            return None
        finally:
    ...
    def batch_update_{{ name }}_byjson(self, jsonstr):
        ...
        try:
            ...
            log.error('Exception at batch_update_{{ name }}_byjson(): %s ' % e)
            if config('app_exception_detail', cast=bool, default=True):
                traceback.print_exc()
            return None
    ...
    def delete_{{ name }}_byid(self, idstr):
        try:
            ...
            with Session(engine) as session:
                statement = select({{ name }}).where(text(idstr))
                result = session.exec(statement).one()
                log.debug('delete_{{ name }}_byid() result is : %s' % result)
                session.delete(result)
                session.commit()
                return True
        except Exception as e:
            log.error('Exception at delete_{{ name }}_byid(): %s ' % e)
            if config('app_exception_detail', cast=bool, default=True):
                traceback.print_exc()
            return False
        finally:
            ...
            try:
                ...
                with Session(engine) as session:
                    ...
                    if count_only:
                        ...
                        log.debug('query_{{ name }}() result is : %s' % result)
                        if include_count:
                            returnjson["record_count"] = record_count
                            ...
                            returnjson['data'] = data
                            returnjson['rawdata'] = rawdata
                    # log.debug('query_{{ name }}() returndict is : %s' % returndict)
                    return returnjson
            except Exception as e:
                log.error('Exception at query_{{ name }}(): %s ' % e)
                if config('app_exception_detail', cast=bool, default=True):
                    traceback.print_exc()
                return None
            finally:
