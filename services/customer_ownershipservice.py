...
from models.customer_ownership import Customer_Ownership
...
class Customer_OwnershipService(object):
    def __init__(self):
        self._modelName = 'Customer_Ownership'

    def new_model(self, modeljson):
        newmodel = Customer_Ownership()
    ...
    def getall_Customer_Ownership(self):
        try:
            engine = dbengine.DBEngine().connect()
            with Session(engine) as session:
                results = session.query(Customer_Ownership).limit(configindb.Query_Config['query_limit_upset'])
                return results.all()
        except Exception as e:
            log.error('Exception at getall_Customer_Ownership(): %s ' % e)
    ...
    def get_Customer_Ownership_count(self):
        try:
            engine = dbengine.DBEngine().connect()
            with Session(engine) as session:
                pks = Customer_Ownership.getPrimaryKeys(Customer_Ownership)
                stmt = text("select count(" + pks[0] + ") as rowcount from Customer_Ownership")
                results = session.execute(stmt)
                return results.one()[0]
        except Exception as e:
            log.error('Exception at get_Customer_Ownership_count(): %s ' % e)
            if config('app_exception_detail', cast=bool, default=True):
                traceback.print_exc()
            return None
        finally:
    ...
    def create_Customer_Ownership(self, modelobj):
        try:
            engine = dbengine.DBEngine().connect()
            with Session(engine) as session:
            ...
            log.error('Exception at create_Customer_Ownership(): %s ' % e)
            if config('app_exception_detail', cast=bool, default=True):
                traceback.print_exc()
            return None
        finally:
    ...
    def batch_create_Customer_Ownership_byjson(self, jsonstr):
        ...
        try:
            for bdata in batchdata:
                ...
                newid = self.create_Customer_Ownership(newentity)
                returnjson['ids'].append(newid)
                returnjson['data'].append(newentity)
            return returnjson
        except Exception as e:
            log.error('Exception at batch_create_Customer_Ownership(): %s ' % e)
            if config('app_exception_detail', cast=bool, default=True):
                traceback.print_exc()
            return None

    def get_Customer_Ownership_byid(self, idstr):
        try:
            engine = dbengine.DBEngine().connect()
            with Session(engine) as session:
                statement = select(Customer_Ownership).where(text(idstr))
                result = session.exec(statement).one()
                #log.debug('get_Customer_Ownership_byid() result is : %s' % result)
                return result
        except Exception as e:
            log.error('Exception at get_Customer_Ownership_byid(): %s ' % e)
            if config('app_exception_detail', cast=bool, default=True):
                traceback.print_exc()
            return None
        finally:
    ...
    def update_Customer_Ownership(self, modelobj):
        try:
            engine = dbengine.DBEngine().connect()
            with Session(engine) as session:
                session.add(modelobj)
            ...
            log.error('Exception at update_Customer_Ownership(): %s ' % e)
            if config('app_exception_detail', cast=bool, default=True):
                traceback.print_exc()
            return None
        finally:
    ...
    def update_Customer_Ownership_byjson(self, updatejson):
        log.debug('The update JSON is: %s' % updatejson)
        pks = Customer_Ownership.getPrimaryKeys(Customer_Ownership)
        statement = select(Customer_Ownership)
        for pk in pks:
            if Customer_Ownership.getpkqmneed(Customer_Ownership,pk):
                statement = statement.where(text("Customer_Ownership." + pk + "='" + str(updatejson[pk])+"'"))
            else:
                statement = statement.where(text("Customer_Ownership."+ pk + "=" +str(updatejson[pk])))
        try:
            engine = dbengine.DBEngine().connect()
            ...
            log.error('Exception at update_Customer_Ownership_byjson(): %s ' % e)
            if config('app_exception_detail', cast=bool, default=True):
                traceback.print_exc()
            return None
        finally:
    ...
    def batch_update_Customer_Ownership_byjson(self, jsonstr):
        batchjson = json.loads(jsonstr)
        batchdata = batchjson['data'] if 'data' in batchjson else None
        returnjson = {"data": []}
        try:
            for bdata in batchdata:
                updentity = self.update_Customer_Ownership_byjson(bdata)
                returnjson['data'].append(updentity)
            return returnjson
        except Exception as e:
            log.error('Exception at batch_update_Customer_Ownership_byjson(): %s ' % e)
            if config('app_exception_detail', cast=bool, default=True):
                traceback.print_exc()
            return None

    def delete_Customer_Ownership_byid(self, idstr):
        try:
            engine = dbengine.DBEngine().connect()
            with Session(engine) as session:
                statement = select(Customer_Ownership).where(text(idstr))
                result = session.exec(statement).one()
                log.debug('delete_Customer_Ownership_byid() result is : %s' % result)
                session.delete(result)
                session.commit()
                return True
        except Exception as e:
            log.error('Exception at delete_Customer_Ownership_byid(): %s ' % e)
            if config('app_exception_detail', cast=bool, default=True):
                traceback.print_exc()
            return False
        finally:
    ...
    def query_Customer_Ownership(self,querystr):
        tablename = "Customer_Ownership"
        if toolkit.validQueryJson(querystr):
            queryjson = json.loads(querystr)
            log.debug('The query JSON is: %s' % queryjson)
            #queryfields
            fullqueryfields = "Customer_Ownership." + ",Customer_Ownership.".join(tuple(Customer_Ownership.__fields__.keys()))
            ...
            try:
                ...
                with Session(engine) as session:
                    ...
                    if include_count | count_only:
                        pks = Customer_Ownership.getPrimaryKeys(Customer_Ownership)
                        qfields = "Customer_Ownership." + ",Customer_Ownership.".join(tuple(pks))
                    ...
                    if count_only:
                        ...
                        log.debug('query_Customer_Ownership() result is : %s' % result)
                        if include_count:
                            returnjson["record_count"] = record_count
                            ...
                            returnjson['data'] = data
                            returnjson['rawdata'] = rawdata
                    # log.debug('query_Customer_Ownership() returndict is : %s' % returndict)
                    return returnjson
            except Exception as e:
                log.error('Exception at query_Customer_Ownership(): %s ' % e)
                if config('app_exception_detail', cast=bool, default=True):
                    traceback.print_exc()
                return None
            finally:
