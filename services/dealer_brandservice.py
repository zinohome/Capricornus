...
from models.dealer_brand import Dealer_Brand
...
class Dealer_BrandService(object):
    def __init__(self):
        self._modelName = 'Dealer_Brand'

    def new_model(self, modeljson):
        newmodel = Dealer_Brand()
    ...
    def getall_Dealer_Brand(self):
        try:
            engine = dbengine.DBEngine().connect()
            with Session(engine) as session:
                results = session.query(Dealer_Brand).limit(configindb.Query_Config['query_limit_upset'])
                return results.all()
        except Exception as e:
            log.error('Exception at getall_Dealer_Brand(): %s ' % e)
    ...
    def get_Dealer_Brand_count(self):
        try:
            engine = dbengine.DBEngine().connect()
            with Session(engine) as session:
                pks = Dealer_Brand.getPrimaryKeys(Dealer_Brand)
                stmt = text("select count(" + pks[0] + ") as rowcount from Dealer_Brand")
                results = session.execute(stmt)
                return results.one()[0]
        except Exception as e:
            log.error('Exception at get_Dealer_Brand_count(): %s ' % e)
            if config('app_exception_detail', cast=bool, default=True):
                traceback.print_exc()
            return None
        finally:
    ...
    def create_Dealer_Brand(self, modelobj):
        try:
            engine = dbengine.DBEngine().connect()
            with Session(engine) as session:
            ...
            log.error('Exception at create_Dealer_Brand(): %s ' % e)
            if config('app_exception_detail', cast=bool, default=True):
                traceback.print_exc()
            return None
        finally:
    ...
    def batch_create_Dealer_Brand_byjson(self, jsonstr):
        ...
        try:
            for bdata in batchdata:
                ...
                newid = self.create_Dealer_Brand(newentity)
                returnjson['ids'].append(newid)
                returnjson['data'].append(newentity)
            return returnjson
        except Exception as e:
            log.error('Exception at batch_create_Dealer_Brand(): %s ' % e)
            if config('app_exception_detail', cast=bool, default=True):
                traceback.print_exc()
            return None

    def get_Dealer_Brand_byid(self, idstr):
        try:
            engine = dbengine.DBEngine().connect()
            with Session(engine) as session:
                statement = select(Dealer_Brand).where(text(idstr))
                result = session.exec(statement).one()
                #log.debug('get_Dealer_Brand_byid() result is : %s' % result)
                return result
        except Exception as e:
            log.error('Exception at get_Dealer_Brand_byid(): %s ' % e)
            if config('app_exception_detail', cast=bool, default=True):
                traceback.print_exc()
            return None
        finally:
    ...
    def update_Dealer_Brand(self, modelobj):
        try:
            engine = dbengine.DBEngine().connect()
            with Session(engine) as session:
                session.add(modelobj)
            ...
            log.error('Exception at update_Dealer_Brand(): %s ' % e)
            if config('app_exception_detail', cast=bool, default=True):
                traceback.print_exc()
            return None
        finally:
    ...
    def update_Dealer_Brand_byjson(self, updatejson):
        log.debug('The update JSON is: %s' % updatejson)
        pks = Dealer_Brand.getPrimaryKeys(Dealer_Brand)
        statement = select(Dealer_Brand)
        for pk in pks:
            if Dealer_Brand.getpkqmneed(Dealer_Brand,pk):
                statement = statement.where(text("Dealer_Brand." + pk + "='" + str(updatejson[pk])+"'"))
            else:
                statement = statement.where(text("Dealer_Brand."+ pk + "=" +str(updatejson[pk])))
        try:
            engine = dbengine.DBEngine().connect()
            ...
            log.error('Exception at update_Dealer_Brand_byjson(): %s ' % e)
            if config('app_exception_detail', cast=bool, default=True):
                traceback.print_exc()
            return None
        finally:
    ...
    def batch_update_Dealer_Brand_byjson(self, jsonstr):
        batchjson = json.loads(jsonstr)
        batchdata = batchjson['data'] if 'data' in batchjson else None
        returnjson = {"data": []}
        try:
            for bdata in batchdata:
                updentity = self.update_Dealer_Brand_byjson(bdata)
                returnjson['data'].append(updentity)
            return returnjson
        except Exception as e:
            log.error('Exception at batch_update_Dealer_Brand_byjson(): %s ' % e)
            if config('app_exception_detail', cast=bool, default=True):
                traceback.print_exc()
            return None

    def delete_Dealer_Brand_byid(self, idstr):
        try:
            engine = dbengine.DBEngine().connect()
            with Session(engine) as session:
                statement = select(Dealer_Brand).where(text(idstr))
                result = session.exec(statement).one()
                log.debug('delete_Dealer_Brand_byid() result is : %s' % result)
                session.delete(result)
                session.commit()
                return True
        except Exception as e:
            log.error('Exception at delete_Dealer_Brand_byid(): %s ' % e)
            if config('app_exception_detail', cast=bool, default=True):
                traceback.print_exc()
            return False
        finally:
    ...
    def query_Dealer_Brand(self,querystr):
        tablename = "Dealer_Brand"
        if toolkit.validQueryJson(querystr):
            queryjson = json.loads(querystr)
            log.debug('The query JSON is: %s' % queryjson)
            #queryfields
            fullqueryfields = "Dealer_Brand." + ",Dealer_Brand.".join(tuple(Dealer_Brand.__fields__.keys()))
            ...
            try:
                ...
                with Session(engine) as session:
                    ...
                    if include_count | count_only:
                        pks = Dealer_Brand.getPrimaryKeys(Dealer_Brand)
                        qfields = "Dealer_Brand." + ",Dealer_Brand.".join(tuple(pks))
                    ...
                    if count_only:
                        ...
                        log.debug('query_Dealer_Brand() result is : %s' % result)
                        if include_count:
                            returnjson["record_count"] = record_count
                            ...
                            returnjson['data'] = data
                            returnjson['rawdata'] = rawdata
                    # log.debug('query_Dealer_Brand() returndict is : %s' % returndict)
                    return returnjson
            except Exception as e:
                log.error('Exception at query_Dealer_Brand(): %s ' % e)
                if config('app_exception_detail', cast=bool, default=True):
                    traceback.print_exc()
                return None
            finally:
