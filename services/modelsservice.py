...
class ModelsService(object):
    ...
    def getall_Models(self):
        try:
            engine = dbengine.DBEngine().connect()
            with Session(engine) as session:
                results = session.query(Models).limit(configindb.Query_Config['query_limit_upset'])
                return results.all()
        except Exception as e:
            log.error('Exception at getall_Models(): %s ' % e)
    ...
    def get_Models_count(self):
        try:
            engine = dbengine.DBEngine().connect()
            with Session(engine) as session:
                pks = Models.getPrimaryKeys(Models)
                stmt = text("select count(" + pks[0] + ") as rowcount from Models")
                results = session.execute(stmt)
                return results.one()[0]
        except Exception as e:
            log.error('Exception at get_Models_count(): %s ' % e)
            if config('app_exception_detail', cast=bool, default=True):
                traceback.print_exc()
            return None
        finally:
    ...
    def create_Models(self, modelobj):
        try:
            engine = dbengine.DBEngine().connect()
            with Session(engine) as session:
            ...
            log.error('Exception at create_Models(): %s ' % e)
            if config('app_exception_detail', cast=bool, default=True):
                traceback.print_exc()
            return None
        finally:
    ...
    def batch_create_Models_byjson(self, jsonstr):
        ...
        try:
            for bdata in batchdata:
                ...
                newid = self.create_Models(newentity)
                returnjson['ids'].append(newid)
                returnjson['data'].append(newentity)
            return returnjson
        except Exception as e:
            log.error('Exception at batch_create_Models(): %s ' % e)
            if config('app_exception_detail', cast=bool, default=True):
                traceback.print_exc()
            return None

    def get_Models_byid(self, idstr):
        try:
            engine = dbengine.DBEngine().connect()
            with Session(engine) as session:
                statement = select(Models).where(text(idstr))
                result = session.exec(statement).one()
                #log.debug('get_Models_byid() result is : %s' % result)
                return result
        except Exception as e:
            log.error('Exception at get_Models_byid(): %s ' % e)
            if config('app_exception_detail', cast=bool, default=True):
                traceback.print_exc()
            return None
        finally:
    ...
    def update_Models(self, modelobj):
        try:
            engine = dbengine.DBEngine().connect()
            with Session(engine) as session:
                session.add(modelobj)
            ...
            log.error('Exception at update_Models(): %s ' % e)
            if config('app_exception_detail', cast=bool, default=True):
                traceback.print_exc()
            return None
        finally:
    ...
    def update_Models_byjson(self, updatejson):
        log.debug('The update JSON is: %s' % updatejson)
        pks = Models.getPrimaryKeys(Models)
        statement = select(Models)
        for pk in pks:
            if Models.getpkqmneed(Models,pk):
                statement = statement.where(text("Models." + pk + "='" + str(updatejson[pk])+"'"))
            else:
                statement = statement.where(text("Models."+ pk + "=" +str(updatejson[pk])))
        try:
            engine = dbengine.DBEngine().connect()
            ...
            log.error('Exception at update_Models_byjson(): %s ' % e)
            if config('app_exception_detail', cast=bool, default=True):
                traceback.print_exc()
            return None
        finally:
    ...
    def batch_update_Models_byjson(self, jsonstr):
        batchjson = json.loads(jsonstr)
        batchdata = batchjson['data'] if 'data' in batchjson else None
        returnjson = {"data": []}
        try:
            for bdata in batchdata:
                updentity = self.update_Models_byjson(bdata)
                returnjson['data'].append(updentity)
            return returnjson
        except Exception as e:
            log.error('Exception at batch_update_Models_byjson(): %s ' % e)
            if config('app_exception_detail', cast=bool, default=True):
                traceback.print_exc()
            return None

    def delete_Models_byid(self, idstr):
        try:
            engine = dbengine.DBEngine().connect()
            with Session(engine) as session:
                statement = select(Models).where(text(idstr))
                result = session.exec(statement).one()
                log.debug('delete_Models_byid() result is : %s' % result)
                session.delete(result)
                session.commit()
                return True
        except Exception as e:
            log.error('Exception at delete_Models_byid(): %s ' % e)
            if config('app_exception_detail', cast=bool, default=True):
                traceback.print_exc()
            return False
        finally:
            ...
            try:
                ...
                with Session(engine) as session:
                    ...
                    if include_count | count_only:
                        pks = Models.getPrimaryKeys(Models)
                        qfields = "Models." + ",Models.".join(tuple(pks))
                    ...
                    if count_only:
                        ...
                        log.debug('query_Models() result is : %s' % result)
                        if include_count:
                            returnjson["record_count"] = record_count
                            ...
                            returnjson['data'] = data
                            returnjson['rawdata'] = rawdata
                    # log.debug('query_Models() returndict is : %s' % returndict)
                    return returnjson
            except Exception as e:
                log.error('Exception at query_Models(): %s ' % e)
                if config('app_exception_detail', cast=bool, default=True):
                    traceback.print_exc()
                return None
            finally:
