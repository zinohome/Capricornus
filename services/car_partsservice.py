...
from models.car_parts import Car_Parts
...
class Car_PartsService(object):
    def __init__(self):
        self._modelName = 'Car_Parts'
    ...
    def getall_Car_Parts(self):
        try:
            engine = dbengine.DBEngine().connect()
            with Session(engine) as session:
                results = session.query(Car_Parts).limit(configindb.Query_Config['query_limit_upset'])
                return results.all()
        except Exception as e:
            log.error('Exception at getall_Car_Parts(): %s ' % e)
    ...
    def get_Car_Parts_count(self):
        try:
            engine = dbengine.DBEngine().connect()
            with Session(engine) as session:
                pks = Car_Parts.getPrimaryKeys(Car_Parts)
                stmt = text("select count(" + pks[0] + ") as rowcount from Car_Parts")
                results = session.execute(stmt)
                return results.one()[0]
        except Exception as e:
            log.error('Exception at get_Car_Parts_count(): %s ' % e)
            if config('app_exception_detail', cast=bool, default=True):
                traceback.print_exc()
            return None
        finally:
    ...
    def create_Car_Parts(self, modelobj):
        try:
            engine = dbengine.DBEngine().connect()
            with Session(engine) as session:
            ...
            log.error('Exception at create_Car_Parts(): %s ' % e)
            if config('app_exception_detail', cast=bool, default=True):
                traceback.print_exc()
            return None
        finally:
    ...
    def batch_create_Car_Parts_byjson(self, jsonstr):
        ...
        try:
            for bdata in batchdata:
                ...
                newid = self.create_Car_Parts(newentity)
                returnjson['ids'].append(newid)
                returnjson['data'].append(newentity)
            return returnjson
        except Exception as e:
            log.error('Exception at batch_create_Car_Parts(): %s ' % e)
            if config('app_exception_detail', cast=bool, default=True):
                traceback.print_exc()
            return None

    def get_Car_Parts_byid(self, idstr):
        try:
            engine = dbengine.DBEngine().connect()
            with Session(engine) as session:
                statement = select(Car_Parts).where(text(idstr))
                result = session.exec(statement).one()
                #log.debug('get_Car_Parts_byid() result is : %s' % result)
                return result
        except Exception as e:
            log.error('Exception at get_Car_Parts_byid(): %s ' % e)
            if config('app_exception_detail', cast=bool, default=True):
                traceback.print_exc()
            return None
        finally:
    ...
    def update_Car_Parts(self, modelobj):
        try:
            engine = dbengine.DBEngine().connect()
            with Session(engine) as session:
                session.add(modelobj)
            ...
            log.error('Exception at update_Car_Parts(): %s ' % e)
            if config('app_exception_detail', cast=bool, default=True):
                traceback.print_exc()
            return None
        finally:
    ...
    def update_Car_Parts_byjson(self, updatejson):
        log.debug('The update JSON is: %s' % updatejson)
        pks = Car_Parts.getPrimaryKeys(Car_Parts)
        statement = select(Car_Parts)
        for pk in pks:
            if Car_Parts.getpkqmneed(Car_Parts,pk):
                statement = statement.where(text("Car_Parts." + pk + "='" + str(updatejson[pk])+"'"))
            else:
                statement = statement.where(text("Car_Parts."+ pk + "=" +str(updatejson[pk])))
        try:
            engine = dbengine.DBEngine().connect()
            ...
            log.error('Exception at update_Car_Parts_byjson(): %s ' % e)
            if config('app_exception_detail', cast=bool, default=True):
                traceback.print_exc()
            return None
        finally:
    ...
    def batch_update_Car_Parts_byjson(self, jsonstr):
        batchjson = json.loads(jsonstr)
        batchdata = batchjson['data'] if 'data' in batchjson else None
        returnjson = {"data": []}
        try:
            for bdata in batchdata:
                updentity = self.update_Car_Parts_byjson(bdata)
                returnjson['data'].append(updentity)
            return returnjson
        except Exception as e:
            log.error('Exception at batch_update_Car_Parts_byjson(): %s ' % e)
            if config('app_exception_detail', cast=bool, default=True):
                traceback.print_exc()
            return None

    def delete_Car_Parts_byid(self, idstr):
        try:
            engine = dbengine.DBEngine().connect()
            with Session(engine) as session:
                statement = select(Car_Parts).where(text(idstr))
                result = session.exec(statement).one()
                log.debug('delete_Car_Parts_byid() result is : %s' % result)
                session.delete(result)
                session.commit()
                return True
        except Exception as e:
            log.error('Exception at delete_Car_Parts_byid(): %s ' % e)
            if config('app_exception_detail', cast=bool, default=True):
                traceback.print_exc()
            return False
        finally:
    ...
    def query_Car_Parts(self,querystr):
        tablename = "Car_Parts"
        if toolkit.validQueryJson(querystr):
            queryjson = json.loads(querystr)
            log.debug('The query JSON is: %s' % queryjson)
            #queryfields
            fullqueryfields = "Car_Parts." + ",Car_Parts.".join(tuple(Car_Parts.__fields__.keys()))
            ...
            try:
                ...
                with Session(engine) as session:
                    ...
                    if include_count | count_only:
                        pks = Car_Parts.getPrimaryKeys(Car_Parts)
                        qfields = "Car_Parts." + ",Car_Parts.".join(tuple(pks))
                    ...
                    if count_only:
                        ...
                        log.debug('query_Car_Parts() result is : %s' % result)
                        if include_count:
                            returnjson["record_count"] = record_count
                            ...
                            returnjson['data'] = data
                            returnjson['rawdata'] = rawdata
                    # log.debug('query_Car_Parts() returndict is : %s' % returndict)
                    return returnjson
            except Exception as e:
                log.error('Exception at query_Car_Parts(): %s ' % e)
                if config('app_exception_detail', cast=bool, default=True):
                    traceback.print_exc()
                return None
            finally:
