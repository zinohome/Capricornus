...
from models.sqlite_sequence import sqlite_sequence
...
class sqlite_sequenceService(object):
    def __init__(self):
        self._modelName = 'sqlite_sequence'

    def new_model(self, modeljson):
        newmodel = sqlite_sequence()
    ...
    def getall_sqlite_sequence(self):
        try:
            engine = dbengine.DBEngine().connect()
            with Session(engine) as session:
                results = session.query(sqlite_sequence).limit(configindb.Query_Config['query_limit_upset'])
                return results.all()
        except Exception as e:
            log.error('Exception at getall_sqlite_sequence(): %s ' % e)
    ...
    def get_sqlite_sequence_count(self):
        try:
            engine = dbengine.DBEngine().connect()
            with Session(engine) as session:
                pks = sqlite_sequence.getPrimaryKeys(sqlite_sequence)
                stmt = text("select count(" + pks[0] + ") as rowcount from sqlite_sequence")
                results = session.execute(stmt)
                return results.one()[0]
        except Exception as e:
            log.error('Exception at get_sqlite_sequence_count(): %s ' % e)
            if config('app_exception_detail', cast=bool, default=True):
                traceback.print_exc()
            return None
        finally:
    ...
    def create_sqlite_sequence(self, modelobj):
        try:
            engine = dbengine.DBEngine().connect()
            with Session(engine) as session:
            ...
            log.error('Exception at create_sqlite_sequence(): %s ' % e)
            if config('app_exception_detail', cast=bool, default=True):
                traceback.print_exc()
            return None
        finally:
    ...
    def batch_create_sqlite_sequence_byjson(self, jsonstr):
        ...
        try:
            for bdata in batchdata:
                ...
                newid = self.create_sqlite_sequence(newentity)
                returnjson['ids'].append(newid)
                returnjson['data'].append(newentity)
            return returnjson
        except Exception as e:
            log.error('Exception at batch_create_sqlite_sequence(): %s ' % e)
            if config('app_exception_detail', cast=bool, default=True):
                traceback.print_exc()
            return None

    def get_sqlite_sequence_byid(self, idstr):
        try:
            engine = dbengine.DBEngine().connect()
            with Session(engine) as session:
                statement = select(sqlite_sequence).where(text(idstr))
                result = session.exec(statement).one()
                #log.debug('get_sqlite_sequence_byid() result is : %s' % result)
                return result
        except Exception as e:
            log.error('Exception at get_sqlite_sequence_byid(): %s ' % e)
            if config('app_exception_detail', cast=bool, default=True):
                traceback.print_exc()
            return None
        finally:
    ...
    def update_sqlite_sequence(self, modelobj):
        try:
            engine = dbengine.DBEngine().connect()
            with Session(engine) as session:
                session.add(modelobj)
            ...
            log.error('Exception at update_sqlite_sequence(): %s ' % e)
            if config('app_exception_detail', cast=bool, default=True):
                traceback.print_exc()
            return None
        finally:
    ...
    def update_sqlite_sequence_byjson(self, updatejson):
        log.debug('The update JSON is: %s' % updatejson)
        pks = sqlite_sequence.getPrimaryKeys(sqlite_sequence)
        statement = select(sqlite_sequence)
        for pk in pks:
            if sqlite_sequence.getpkqmneed(sqlite_sequence,pk):
                statement = statement.where(text("sqlite_sequence." + pk + "='" + str(updatejson[pk])+"'"))
            else:
                statement = statement.where(text("sqlite_sequence."+ pk + "=" +str(updatejson[pk])))
        try:
            engine = dbengine.DBEngine().connect()
            ...
            log.error('Exception at update_sqlite_sequence_byjson(): %s ' % e)
            if config('app_exception_detail', cast=bool, default=True):
                traceback.print_exc()
            return None
        finally:
    ...
    def batch_update_sqlite_sequence_byjson(self, jsonstr):
        batchjson = json.loads(jsonstr)
        batchdata = batchjson['data'] if 'data' in batchjson else None
        returnjson = {"data": []}
        try:
            for bdata in batchdata:
                updentity = self.update_sqlite_sequence_byjson(bdata)
                returnjson['data'].append(updentity)
            return returnjson
        except Exception as e:
            log.error('Exception at batch_update_sqlite_sequence_byjson(): %s ' % e)
            if config('app_exception_detail', cast=bool, default=True):
                traceback.print_exc()
            return None

    def delete_sqlite_sequence_byid(self, idstr):
        try:
            engine = dbengine.DBEngine().connect()
            with Session(engine) as session:
                statement = select(sqlite_sequence).where(text(idstr))
                result = session.exec(statement).one()
                log.debug('delete_sqlite_sequence_byid() result is : %s' % result)
                session.delete(result)
                session.commit()
                return True
        except Exception as e:
            log.error('Exception at delete_sqlite_sequence_byid(): %s ' % e)
            if config('app_exception_detail', cast=bool, default=True):
                traceback.print_exc()
            return False
        finally:
    ...
    def query_sqlite_sequence(self,querystr):
        ...
        if toolkit.validQueryJson(querystr):
            ...
            #queryfields
            fullqueryfields = "sqlite_sequence." + ",sqlite_sequence.".join(tuple(sqlite_sequence.__fields__.keys()))
            ...
            try:
                ...
                with Session(engine) as session:
                    ...
                    if include_count | count_only:
                        pks = sqlite_sequence.getPrimaryKeys(sqlite_sequence)
                        qfields = "sqlite_sequence." + ",sqlite_sequence.".join(tuple(pks))
                    ...
                    if count_only:
                        ...
                        log.debug('query_sqlite_sequence() result is : %s' % result)
                        if include_count:
                            returnjson["record_count"] = record_count
                            ...
                            returnjson['data'] = data
                            returnjson['rawdata'] = rawdata
                    # log.debug('query_sqlite_sequence() returndict is : %s' % returndict)
                    return returnjson
            except Exception as e:
                log.error('Exception at query_sqlite_sequence(): %s ' % e)
                if config('app_exception_detail', cast=bool, default=True):
                    traceback.print_exc()
                return None
            finally:
